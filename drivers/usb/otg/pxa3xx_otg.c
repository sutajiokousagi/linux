...
#if defined(CONFIG_MACH_ASPENITE) || defined(CONFIG_MACH_ZYLONITE2)\
		|| defined(CONFIG_MACH_TETON_BGA)
	if (machine_is_aspenite() || machine_is_zylonite2() ||
			machine_is_teton_bga()) {
		...
		if (info->init_pmic_ops) {
			pxa_otg->pmic_ops = (struct otg_pmic_ops *)info->init_pmic_ops();
		}
	}
#endif
