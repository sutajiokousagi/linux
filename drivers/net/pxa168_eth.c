...
static void pxa168_eth_set_rx_mode(struct net_device *dev)
{
	...
	for (i = 0; i < dev->mc_count; i++) {
		update_hash_table_mac_address(mp, NULL, mclist->dmi_addr);
		mclist = mclist->next;
	}
}
...
static int pxa168_eth_open(struct net_device *dev)
{
	...
	if (err) {
		printk(KERN_ERR "Can not assign IRQ number to PXA168_eth\n");
		return -EAGAIN;
	}
#endif
...
static const struct net_device_ops pxa168_netdev_ops = {
	...
	.ndo_set_multicast_list	 = pxa168_eth_set_rx_mode,
