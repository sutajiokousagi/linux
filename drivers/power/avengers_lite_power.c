...
static int avengers_lite_read_status(struct avengers_lite_device_info *di)
{
	...
	if (di->STATUS != EC_NO_CONNECTION) {
		...
		if (temp_byte != di->BATT_ABSENT) {
			printk(KERN_ALERT "battery present event change"
					...
					union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS: /*1--charging 2--discharging 4--Full 0--Unkhown 3--Not charging*/
		if (di->STATUS == EC_NO_CONNECTION)
			...
			if (di->RARC > 95)
				val->intval = 4;
			/* report unknown status if battery not present */
			if (di->BATT_ABSENT)
				val->intval = 0;
		}
...
static enum power_supply_property avengers_lite_ac_props[] = {
	POWER_SUPPLY_PROP_ONLINE,
};

static int avengers_lite_ac_read_status(struct avengers_lite_device_info *di)
{
	int ret;
	unsigned char temp_byte;
...
static int avengers_lite_ac_get_property(struct power_supply *psy,
				  enum power_supply_property psp,
				  union power_supply_propval *val)
{
	struct avengers_lite_device_info *di = to_avengers_lite_device_info(psy);

	avengers_lite_ac_read_status(di);
...
static int avengers_lite_power_probe(struct platform_device *pdev)
{
	...
	di->ac.properties = avengers_lite_ac_props;
	di->ac.num_properties = ARRAY_SIZE(avengers_lite_ac_props);
	di->ac.get_property = avengers_lite_ac_get_property;
	di->ac.name = "ac";
	...
	di->monitor_wqueue = create_singlethread_workqueue("avlite power");
